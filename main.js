...
// create global variable brushedPoints
let brushedPoints = [];
...
//resetting brushedPoints2 to an empty array
let brushedPoints2 = []
  ...
  if (extent){
   myCircle2.classed("selected", function(d){
     ...
     if (extent[0][0] <= cx && cx <= extent[1][0] && extent[0][1] <= cy && cy <= extent[1][1]) {
      //append the id of the selected circle to an array called brushedPoints2
       brushedPoints2.push(id);
       //run brushedPushed2 through the function "selectOthers"
...
d3.csv("data/iris.csv").then((data) => {
 ...
 const Y_SCALE_LENGTH = d3.scaleLinear()
   ...
   //plot points
   const myCircle1 = FRAME1.selectAll("circles") 
     .data(data) // passed from .then 
     .enter()      
     .append("circle") 
     .attr("cx", (d) => { return (X_SCALE_LENGTH(d.Sepal_Length) + MARGINS.left); })
  ...
  //filter through myCircle and change the style of the points if their IDs are included in brushedPoints2
